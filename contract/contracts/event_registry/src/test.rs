...
#[test]
fn test_register_event_duplicate_tier_id_fails() {
    let env = Env::default();
    let contract_id = env.register(EventRegistry, ());
    let client = EventRegistryClient::new(&env, &contract_id);
    ...
    // Build a map with a duplicate tier ID — Soroban's Map silently keeps the
    // last value, so the map ends up with one entry instead of two.
    let tier = TicketTier {
        name: String::from_str(&env, "General"),
        price: 1_000,
        tier_limit: 50,
    ...
    let mut tiers = Map::new(&env);
    tiers.set(String::from_str(&env, "general"), tier.clone());
    // Setting the same key again — this is the duplicate
    tiers.set(String::from_str(&env, "general"), tier);

    let result = client.try_register_event(&EventRegistrationArgs {
        event_id: String::from_str(&env, "event_dup_tier"),
        name: String::from_str(&env, "Dup Tier Event"),
        organizer_address: organizer,
        payment_address: Address::generate(&env),
        metadata_cid,
        max_supply: 0,
        milestone_plan: None,
        tiers,
        // Declare two IDs but the map only has one (duplicate was collapsed)
        tier_ids: soroban_sdk::vec![
            &env,
            String::from_str(&env, "general"),
            String::from_str(&env, "general"),
        ],
    ...
    assert_eq!(result, Err(Ok(EventRegistryError::DuplicateTierId)));
}
